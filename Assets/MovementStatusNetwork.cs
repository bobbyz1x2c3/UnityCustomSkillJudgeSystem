...
public class MovementStatusNetwork : NetworkBehaviour
{
    [SerializeField]
    ...
    {
        if (IsOwner && IsClient)
            UpdateTargetSpeedServerRpc(Camera.eulerAngles.y);
        
    }

    [ServerRpc]
    void UpdateTargetSpeedServerRpc(float _canmera_transform_rotation_eulerAngles_y)
    {
        ...
        if (lastInputFaceValue != Vector2.zero || focusMode) 
            ...
            TargetFaceAngle = FixAngle(_canmera_transform_rotation_eulerAngles_y - Vector2.SignedAngle(Vector2.up, lastInputFaceValue));
